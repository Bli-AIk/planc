...
1. Run `node <skill>/scripts/planc.cjs init <project>` once. The explicit initialization exception appends `/.plan/` to the outer `.gitignore` and creates independent local Git history in `.plan`. It preserves existing files and configuration; no submodule, remote, or push.
