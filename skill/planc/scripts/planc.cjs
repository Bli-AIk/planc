...
var require_paths = __commonJS({
  "src/lib/paths.js"(exports2, module2) {
    "use strict";
    var fs = require("node:fs");
    var path2 = require("node:path");
    function projectRoot(root = process.cwd()) {
      return fs.realpathSync(path2.resolve(root));
    }
    function stat(file) {
      try {
    ...
    function planRoot(root = process.cwd(), optional = false) {
      const dir = path2.join(projectRoot(root), ".plan");
      assertLocal(dir, true, optional);
      return dir;
    }
    function planFile(root) {
      return path2.join(planRoot(root), "plan.json");
    }
    ...
    function noteFile(root, name) {
      ...
      for (let i = 0; i < parts.length; i++) {
        file = path2.join(file, parts[i]);
        assertLocal(file, i < parts.length - 1);
      }
      return file;
    }
    function auditTree(dir, { git = false, top = true } = {}) {
      ...
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) auditTree(file, { git, top: false });
        else assertLocal(file);
      }
    }
    module2.exports = { projectRoot, planRoot, planFile, assertLocal, assertNoteName, noteFile, auditTree };
  }
});
...
var require_schema = __commonJS({
  "src/schema.json"(exports2, module2) {
    module2.exports = {
      $schema: "http://json-schema.org/draft-07/schema#",
      $id: "https://planc.local/schema/v1",
      type: "object",
      additionalProperties: false,
      required: ["version", "title", "updatedAt", "tasks", "relations", "graphs", "checks", "changes"],
      properties: {
        version: { const: 1 },
      ...
      definitions: {
        ...
        notes: { type: "array", uniqueItems: true, items: { type: "string", pattern: "^notes/.+\\.md$" } },
        task: {
          type: "object",
          additionalProperties: false,
          required: ["id", "title", "kind", "goal", "completionCriteria", "status", "notes"],
          properties: {
            id: { $ref: "#/definitions/id" },
            title: { $ref: "#/definitions/text" },
            kind: { enum: ["implementation", "understanding", "investigation", "decision"] },
            goal: { $ref: "#/definitions/text" },
            completionCriteria: { type: "array", minItems: 1, items: { $ref: "#/definitions/text" } },
            status: { enum: ["not_started", "in_progress", "completed"] },
            notes: { $ref: "#/definitions/notes" },
            completion: { type: "object", additionalProperties: false, required: ["checkId"], properties: { checkId: { $ref: "#/definitions/id" } } }
          }
        },
        relation: {
          type: "object",
          ...
          properties: {
            ...
            type: { enum: ["prerequisite", "related"] },
        ...
        graph: {
          type: "object",
          additionalProperties: false,
          required: ["id", "title", "taskIds", "notes"],
          properties: {
            id: { $ref: "#/definitions/id" },
            title: { $ref: "#/definitions/text" },
            taskIds: { type: "array", uniqueItems: true, items: { $ref: "#/definitions/id" } },
            notes: { $ref: "#/definitions/notes" }
          }
        },
        check: {
          type: "object",
          additionalProperties: false,
          required: ["id", "taskId", "at", "kind", "outcome", "summary", "evidence"],
          properties: {
            id: { $ref: "#/definitions/id" },
            taskId: { $ref: "#/definitions/id" },
            at: { $ref: "#/definitions/timestamp" },
            kind: { enum: ["review", "user_confirmation"] },
            outcome: { enum: ["completed", "needs_work"] },
            summary: { $ref: "#/definitions/text" },
            evidence: { type: "array", minItems: 1, items: { $ref: "#/definitions/text" } },
            dissent: { $ref: "#/definitions/text" }
          },
          allOf: [{ if: { properties: { kind: { const: "user_confirmation" } } }, then: { required: ["dissent"], properties: { outcome: { const: "completed" } } } }]
        },
        change: {
          type: "object",
          additionalProperties: false,
          required: ["id", "at", "reason", "summary", "taskIds"],
          properties: {
            id: { $ref: "#/definitions/id" },
            at: { $ref: "#/definitions/timestamp" },
...
var require_plan = __commonJS({
  "src/lib/plan.js"(exports2, module2) {
    "use strict";
    var fs = require("node:fs");
    var crypto = require("node:crypto");
    var Ajv = require_ajv();
    var { planFile, assertLocal, assertNoteName, noteFile } = require_paths();
    var ajv = new Ajv({ allErrors: true });
    ajv.addFormat("date-time", (value) => /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.\d+)?(?:Z|[+-]\d\d:\d\d)$/.test(value) && Number.isFinite(Date.parse(value)));
    var validateSchema = ajv.compile(require_schema());
    function emptyPlan(title = "My project") {
      return { version: 1, title, updatedAt: (/* @__PURE__ */ new Date()).toISOString(), tasks: [], relations: [], graphs: [{ id: "main", title: "Main", taskIds: [], notes: [] }], checks: [], changes: [] };
    }
    ...
    function noteNames(plan) {
      return [...new Set([...plan.tasks, ...plan.graphs].flatMap((item) => item.notes))].sort();
    }
    function validatePlan(plan) {
      ...
      for (const key of ["tasks", "relations", "graphs", "checks", "changes"]) {
        ...
        for (const item of plan[key]) {
          if (seen.has(item.id)) errors.push(`Duplicate ${key} id: ${item.id}`);
          seen.add(item.id);
        }
      }
      ...
      for (const rel of plan.relations) {
        ...
        if (relations.has(key)) errors.push(`Duplicate relation: ${rel.from} -> ${rel.to}`);
        relations.add(key);
        if (rel.type === "prerequisite" && tasks.has(rel.from) && tasks.has(rel.to)) {
          degrees.set(rel.to, degrees.get(rel.to) + 1);
      ...
      for (const name of noteNames(plan)) try {
        assertNoteName(name);
      } catch (error) {
    ...
    function enrichPlan(plan) {
      return { ...plan, tasks: plan.tasks.map((t) => ({ ...t, readiness: readiness(plan, t.id) })) };
    }
    ...
    module2.exports = { emptyPlan, readPlan, validatePlan, readiness, enrichPlan, visibleTasks, loadSnapshot: loadSnapshot2, noteNames };
  }
});

// src/lib/workspace.js
var require_workspace = __commonJS({
  "src/lib/workspace.js"(exports2, module2) {
    "use strict";
    var fs = require("node:fs");
    var path2 = require("node:path");
    var { execFileSync } = require("node:child_process");
    var { projectRoot, planRoot, assertLocal, auditTree } = require_paths();
    var { emptyPlan, loadSnapshot: loadSnapshot2 } = require_plan();
    ...
    function assertIndependentGit(dir) {
      assertLocal(path2.join(dir, ".git"), true);
      auditTree(path2.join(dir, ".git"), { git: true });
      const top = git(dir, ["rev-parse", "--show-toplevel"]).trim();
      const common = git(dir, ["rev-parse", "--path-format=absolute", "--git-common-dir"]).trim();
      if (path2.resolve(top) !== dir || path2.resolve(common) !== path2.join(dir, ".git")) throw new Error(".plan must be an independent Git repository");
      if (fs.existsSync(path2.join(dir, ".git/objects/info/alternates"))) throw new Error("External Git object stores are not supported in .plan");
    }
    function checkpoint2(root, message = "Update plan") {
      const dir = planRoot(root);
      const snapshot = loadSnapshot2(root);
      auditTree(dir);
      assertIndependentGit(dir);
      function checkAttributes(folder) {
        for (const entry of fs.readdirSync(folder, { withFileTypes: true })) {
          ...
          if (entry.isDirectory()) checkAttributes(path2.join(folder, entry.name));
        }
      }
      checkAttributes(dir);
      if (fs.existsSync(path2.join(dir, ".git/info/attributes"))) throw new Error("Git info/attributes is not supported in .plan");
      git(dir, ["add", "--all", "--", "."]);
    ...
    function init2(root = process.cwd()) {
      ...
      const file = path2.join(dir, "plan.json");
      if (!assertLocal(file, false, true)) fs.writeFileSync(file, `${JSON.stringify(emptyPlan(path2.basename(root)), null, 2)}
`, { flag: "wx" });
      ...
      if (!fs.existsSync(path2.join(dir, ".git"))) git(dir, ["init", "--quiet", "--template="]);
      return checkpoint2(root, "Initialize plan");
    }
    function history(root) {
      ...
      try {
        return git(dir, ["log", "-30", "-z", "--format=%H%x00%aI%x00%s"]).split("\0").reduce((items, value, i, fields) => {
          if (i % 3 === 0 && value) items.push({ id: value, at: fields[i + 1], message: fields[i + 2] });
          return items;
        }, []);
      } catch (error) {
    ...
    module2.exports = { init: init2, checkpoint: checkpoint2, history, assertIndependentGit, git };
  }
});
...
var require_server = __commonJS({
  "src/server.js"(exports2, module2) {
    ...
    var fs = require("node:fs");
    var path2 = require("node:path");
    var { loadSnapshot: loadSnapshot2 } = require_plan();
    var { planRoot, projectRoot } = require_paths();
    var { history } = require_workspace();
    async function startServer2(root = process.cwd(), options = {}) {
      root = projectRoot(root);
      planRoot(root);
      ...
      ].map(([url, [file, type]]) => [url, { body: fs.readFileSync(path2.join(__dirname, "../dist", file)), type }]));
      function refresh() {
        try {
          lastGood = loadSnapshot2(root);
          payload = { ok: true, snapshot: lastGood };
        } catch (error) {
          payload = { ok: false, errors: [error.message], snapshot: lastGood };
        }
        const next = JSON.stringify(payload);
        if (next !== serialized) {
          serialized = next;
          for (const res of clients) res.write(`event: plan
data: ${serialized}
...
`);
      ...
      const server = http.createServer((req, res) => {
        ...
        if (url.pathname === "/api/plan") {
          refresh();
          return send(payload.ok ? 200 : 422, payload);
        }
...
`);
        ...
        if (url.pathname === "/api/history") {
          try {
            return send(200, { commits: history(root) });
          } catch (error) {
            return send(422, { error: error.message });
          }
        }
    ...
    module2.exports = { startServer: startServer2 };
  }
});
...
var { init, checkpoint } = require_workspace();
var { loadSnapshot } = require_plan();
...
async function main(args = process.argv.slice(2)) {
  try {
    ...
    if (command === "validate") {
      const { plan } = loadSnapshot(root);
      console.log(`Valid: ${plan.tasks.length} tasks, ${plan.graphs.length} graphs, ${plan.checks.length} checks`);
      return 0;
    }
    if (command === "checkpoint") {
      const result = checkpoint(root, values.message);
      console.log(result.changed ? `Committed ${result.commit.slice(0, 8)}` : "No changes to commit.");
      return 0;
    }
    if (command === "serve") {
      let port = values.port === void 0 ? 4317 : Number(values.port);
      let result;
      for (let attempts = 0; ; attempts++) {
        try {
          result = await startServer(root, { port });
          break;
        } catch (error) {
          if (error.code !== "EADDRINUSE" || attempts >= 20 || port === 65535) throw error;
          port++;
        }
      }
      console.log(`planc serving ${result.url}`);
      const stop = () => result.close().then(() => {
        process.exitCode = 0;
      });
