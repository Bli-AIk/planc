...
async function main(args = process.argv.slice(2)) {
  try {
    const { values, positionals } = parseArgs({ args, allowPositionals: true, options: {
      port: { type: 'string' }, message: { type: 'string', short: 'm' }, 'accept-plan-git': { type: 'boolean' }, help: { type: 'boolean', short: 'h' }
    } });
    const [command, directory] = positionals;
    if (values.help || !command) {
      console.log('planc 0.1.0\n\n  planc init [project-dir] [--accept-plan-git]\n  planc validate [project-dir]\n  planc serve [project-dir] [--port 4317]\n  planc checkpoint [project-dir] [-m "Update plan"]\n\nNode >=22 and Git are required. init creates independent .plan Git history only after --accept-plan-git. serve is read-only and binds to 127.0.0.1.'); return 0;
    }
    ...
    if (command === 'init') { const result = init(root, { acceptPlanGit: values['accept-plan-git'] }); console.log(`Initialized ${path.join(root, '.plan')} (${result.commit.slice(0, 8)})`); return 0; }
