...
test('init requires explicit consent before creating independent plan Git history', t => {
  const root = temporary(t);
  assert.throws(() => init(root), /--accept-plan-git/);
  assert.equal(fs.existsSync(path.join(root, '.plan')), false);
  assert.equal(fs.existsSync(path.join(root, '.gitignore')), false);
  init(root, { acceptPlanGit: true });
...
test('init is idempotent, preserves existing files/config, and isolates Git from an outer repository', t => {
  ...
  init(root, { acceptPlanGit: true }); const first = history(root); const before = fs.readFileSync(path.join(root, '.plan/plan.json'), 'utf8');
...
test('init repairs a later negation by appending the ignore rule once', t => {
  ...
  init(root, { acceptPlanGit: true }); init(root); assert.equal(fs.readFileSync(path.join(root, '.gitignore'), 'utf8'), '/.plan/\n!/.plan/\n/.plan/\n');
});
...
test('symlinks and linked worktrees cannot redirect initialization or checkpoints', t => {
  ...
  fs.rmSync(path.join(root, '.gitignore')); init(root, { acceptPlanGit: true });
...
test('local Git worktree redirection and hooks cannot write to outer project', t => {
  const root = temporary(t); init(root, { acceptPlanGit: true }); const dir = path.join(root, '.plan');
...
test('standalone Skill runs without package dependencies or writes outside its target .plan', t => {
  ...
  for (const command of [['init', '--accept-plan-git'], ['validate'], ['checkpoint']]) {
    const result = spawnSync(process.execPath, [cli, command[0], root, ...(command.slice(1))], { cwd: isolated, encoding: 'utf8' });
